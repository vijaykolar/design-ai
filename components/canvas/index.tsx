...
import { useCopyFrame } from "@/features/use-frame";
...
const Canvas = ({
  ...
  const copyFrameMutation = useCopyFrame(projectId);
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Ctrl+C / Cmd+C: Copy selected frame
      if ((e.metaKey || e.ctrlKey) && e.key === "c" && selectedFrameId) {
        e.preventDefault();
        setCopiedFrameId(selectedFrameId);
        toast.success("Frame copied", { duration: 1000 });
      }

      // Ctrl+V / Cmd+V: Paste copied frame
      if ((e.metaKey || e.ctrlKey) && e.key === "v" && copiedFrameId) {
        e.preventDefault();
        copyFrameMutation.mutate(copiedFrameId);
      }
  ...
  }, [toolMode, setSelectedFrameId, handleCanvasScreenshot, selectedFrameId, copiedFrameId, copyFrameMutation]);
                  ...
                  >
                    <div>
                      {frames?.map((frame, index: number) => {
                        ...
                        const baseX = frame.x !== undefined ? frame.x : 100 + index * 480;
                        const y = frame.y !== undefined ? frame.y : 100;
